...
{
    if (flags.Disps13)
        variants_.push_back({3, 1, {0.76561395265924, 0.2066991671971, 0.027686880143657}, 10.3});

    if (flags.Disps15)
        variants_.push_back({5, 1, {0.49900087977418, 0.33439107198377, 0.15517028825608,
                                      0.011387362618405, 0.50397367568471e-4}, 10.3});

    if (flags.Disps23)
        variants_.push_back({3, 2, {0.25, 0.461290700433682, 0.28870929566318}, 12.0});

    if (flags.Disps25)
        variants_.push_back({5, 2, {0.25, 0.39269911064722, 0.2811818347201,
                                      0.06530730655278, 0.010811748097894}, 12.0});
